 ...
 *
 * @section DESCRIPTION
 * Common functions for the CPU_MSD library.
 */
#include <cstring>
#include <string>
...
namespace SF {
	namespace MSD {
		...
		bool getConfigParameter (const string &cfgFile, const char *param, string &result)
		{
			...
			if (strcmp (reinterpret_cast< const char* > (node->name), "SFMSDInfo")){
				PRINT ("error: root element in %s not of \'SFMSDInfo\' type", cfgFile.c_str ());
				xmlFreeDoc (doc);
				return false;
			}
