...
namespace SF {
	...
	static inline size_t getCellSubmeshIndex (const vector <FEMSubmesh> &submesh, const vec3 &v1, const vec3 &v2, const vec3 &v3, const vec3 &v4)
	{
		...
		for (size_t i = 0; i < submesh.size (); ++i){
			...
			if (submesh.at (i)._bbox.collide (v2)){
				if (counter){
					return i;
				}
				++counter;
			}
			if (submesh.at (i)._bbox.collide (v3)){
				if (counter){
					return i;
				}
				++counter;
			}
			if (submesh.at (i)._bbox.collide (v4)){
				if (counter){
					return i;
				}
			}
	...
	FEMMesh::process (const int depth)
	{
      ...
      _vertInfo.resize (_vertices.size ());
      if (depth){
        size_t submeshIndex, cellIndex;
        for (size_t i = 0; i < _cells.size (); i += 4){
          ...
          cellIndex = _submesh [submeshIndex]._cells.size ()/ 4;
          for (int j = 0; j < 4; ++j){
            _vertInfo [_cells [i + j]].addOwner (submeshIndex, cellIndex);
          }
          for (int j = 0; j < 4; ++j){
        ...
        for (unsigned int i = 0; i < _cells.size (); i+= 4){
          for (unsigned int j = 0; j < 4; ++j){
            _vertInfo [_cells[i + j]].addOwner (0, i/ 4);
            _submesh [0]._cells.push_back (_cells [i + j]);
          }
        }
      }
			_cells.clear ();
	...
	FEMMesh::shuffleVertices ()
	{
		...
		if (_submesh.size () > 1){
			...
			for (int i = sOffset.size () - 1; i > 0; --i){
			...
			for (int i = iOffset.size () - 1; i > 0; --i){
	...
	FEMMesh::writeElementsToFiles (const string &folder, const string &prefix) const
	{
		...
		{
		  fname = folder + prefix;
		  fname.append (".node.own");

		  fp = fopen (fname.c_str (), "w");
		  assert (fp);

		  fprintf (fp, "%lu\n", _vertInfo.size ());
		  for (unsigned int i = 0; i < _vertInfo.size (); ++i){
        fprintf (fp, "%u", _vertInfo [i]._nOwners);
        for (unsigned int j = 0; j < _vertInfo [i]._nOwners; ++j){
          fprintf (fp, " %u %u", _vertInfo [i]._owner [j]._submesh, _vertInfo [i]._owner [j]._cellIndex);
        }
        fprintf (fp, "\n");
		  }
		...
		// write external triangle owner info to files
		{
		  vector <CellOwner> *cptr;
			for (size_t i = 0; i < _submesh.size (); ++i){
				...
				ss << i;
				fname.append (ss.str ());
				fname.append (".trio.own");

				fp = fopen (fname.c_str (), "w");
				assert (fp);

				cptr = const_cast <vector <CellOwner>* > (&(_submesh. at(i)._efown));
				fprintf (fp, "%lu\n", cptr->size ());

				for (size_t j = 0; j < cptr->size (); ++j){
					fprintf (fp, "%u %u\n", cptr->at (j)._owner, cptr->at (j)._index);
				}
		...
		// write internal triangle owner info to files
		{
		  vector <CellOwner> *cptr;
			for (size_t i = 0; i < _submesh.size (); ++i){
				...
				ss << i;
				fname.append (ss.str ());
				fname.append (".trii.own");

				fp = fopen (fname.c_str (), "w");
				assert (fp);

				cptr = const_cast <vector <CellOwner>* > (&(_submesh. at(i)._ifown));
				fprintf (fp, "%lu\n", cptr->size ());
