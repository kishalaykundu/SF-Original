...
static void display_usage ()
{
	...
	cerr << "\t-p [--file-prefix] <prefix>\tfile prefix of tetrahedron mesh file" << endl;
...
main (int argc, char* argv  [])
{
...

	{
			...
			if (!strcmp (argv [index], "-i") || !strcmp (argv [index], "--input-dir")){
				...
				for (unsigned int i = 0; i < strlen (argv [index]); ++i){
					if (!isdigit (argv [index] [i])){
					  cerr << "error: invalid -d option: " << argv [index] << endl;
					  display_usage ();
					  exit (EXIT_FAILURE);
					}
				}
				...
				for (int i = 0; i < 3; ++i){
					++index;
					for (unsigned int j = 0; j < strlen (argv [index]); ++j){
						if (!isdigit (argv [index] [j]) && argv [index] [j] != '.'){
              cerr << "error: invalid -e option: " << argv [index] << endl;
              display_usage ();
              exit (EXIT_FAILURE);
						}
					}
		...
		orderCells (reverseflag, startcell, mptr->_cells, mptr->_faces);
	}
